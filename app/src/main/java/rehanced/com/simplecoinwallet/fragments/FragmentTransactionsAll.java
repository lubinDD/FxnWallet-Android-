        ...
        if(storedwallets.size() == 0){
            ...
            for (int i = 0; i < storedwallets.size(); i++) {
                try {
                    ...
                    FCoinAPI.getInstance().getNormalTransactions(currentWallet.getPubKey(), new Callback() {
                        @Override
                    ...
                    FCoinAPI.getInstance().getInternalTransactions(currentWallet.getPubKey(), new Callback() {
                        @Override
                        public void onFailure(Call call, IOException e) {
                            if(isAdded()) {
                                ac.runOnUiThread(new Runnable() {
