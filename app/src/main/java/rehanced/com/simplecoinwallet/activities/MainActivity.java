package rehanced.com.simplecoinwallet.activities;
...
import rehanced.com.simplecoinwallet.R;
import rehanced.com.simplecoinwallet.data.WatchWallet;
import rehanced.com.simplecoinwallet.fragments.FragmentPrice;
import rehanced.com.simplecoinwallet.fragments.FragmentTransactionsAll;
import rehanced.com.simplecoinwallet.fragments.FragmentWallets;
import rehanced.com.simplecoinwallet.interfaces.NetworkUpdateListener;
import rehanced.com.simplecoinwallet.services.NotificationLauncher;
import rehanced.com.simplecoinwallet.services.WalletGenService;
import rehanced.com.simplecoinwallet.utils.AddressNameConverter;
import rehanced.com.simplecoinwallet.utils.Dialogs;
import rehanced.com.simplecoinwallet.utils.ExchangeCalculator;
import rehanced.com.simplecoinwallet.utils.ExternalStorageHandler;
import rehanced.com.simplecoinwallet.utils.OwnWalletUtils;
import rehanced.com.simplecoinwallet.utils.Settings;
import rehanced.com.simplecoinwallet.utils.WalletStorage;
...
public class MainActivity extends SecureAppCompatActivity implements NetworkUpdateListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(preferences.getLong("APP_INSTALLED", 0) == 0){
            Intent intro = new Intent(this, rehanced.com.simplecoinwallet.activities.AppIntroActivity.class);
            startActivityForResult(intro, rehanced.com.simplecoinwallet.activities.AppIntroActivity.REQUEST_CODE);
        }
        ...
        AccountHeader headerResult = new AccountHeaderBuilder()
                .withActivity(this)
                .withHeaderBackground(R.drawable.fxnow_bg)
                .build();
        ...
        //fragments[0] = new FragmentPrice();
        fragments[0] = new FragmentWallets();
        fragments[1] = new FragmentTransactionsAll();
        //**********************************************************
            ...
            case ExternalStorageHandler.REQUEST_WRITE_STORAGE: {
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    if(fragments != null && fragments[0] != null)
                        ((FragmentWallets)fragments[0]).export();
                } else {
    ...
    public void onResume(){
        ...
        if(fragments != null && fragments[0] != null && WalletStorage.getInstance(this).get().size() != ((FragmentWallets)fragments[0]).getDisplayedWalletCount()){
            try {
                ((FragmentWallets)fragments[0]).update();
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    public void onActivityResult(int requestCode, int resultCode, final Intent data) {
        ...
        if (requestCode == rehanced.com.simplecoinwallet.activities.QRScanActivity.REQUEST_CODE) {
            if (resultCode == RESULT_OK) {
                byte type = data.getByteExtra("TYPE", rehanced.com.simplecoinwallet.activities.QRScanActivity.SCAN_ONLY);
                if (type == rehanced.com.simplecoinwallet.activities.QRScanActivity.SCAN_ONLY) {
                    if(data.getStringExtra("ADDRESS").length() != 42 || ! data.getStringExtra("ADDRESS").startsWith("0x")) {
                        snackError("Invalid Ethereum address!");
                        return;
                    }
                    Intent watch = new Intent(this, rehanced.com.simplecoinwallet.activities.AddressDetailActivity.class);
                    watch.putExtra("ADDRESS", data.getStringExtra("ADDRESS"));
                    startActivity(watch);
                } else if (type == rehanced.com.simplecoinwallet.activities.QRScanActivity.ADD_TO_WALLETS) {
                    if(data.getStringExtra("ADDRESS").length() != 42 || ! data.getStringExtra("ADDRESS").startsWith("0x")) {
                        snackError("Invalid Ethereum address!");
                        return;
                    }
                ...
                } else if (type == rehanced.com.simplecoinwallet.activities.QRScanActivity.REQUEST_PAYMENT) {
                    if (WalletStorage.getInstance(this).getFullOnly().size() == 0) {
                        Dialogs.noFullWallet(this);
                    } else {
                        Intent watch = new Intent(this, rehanced.com.simplecoinwallet.activities.SendActivity.class);
                        watch.putExtra("TO_ADDRESS", data.getStringExtra("ADDRESS"));
                        watch.putExtra("AMOUNT", data.getStringExtra("AMOUNT"));
                        startActivity(watch);
                    }
                } else if (type == rehanced.com.simplecoinwallet.activities.QRScanActivity.PRIVATE_KEY) {
        ...
        } else if (requestCode == rehanced.com.simplecoinwallet.activities.WalletGenActivity.REQUEST_CODE) {
            if (resultCode == RESULT_OK) {
        ...
        } else if (requestCode == rehanced.com.simplecoinwallet.activities.SendActivity.REQUEST_CODE) {
            if (resultCode == RESULT_OK) {
                if (fragments == null || fragments[1] == null) return;
                ((FragmentTransactionsAll) fragments[1]).addUnconfirmedTransaction(data.getStringExtra("FROM_ADDRESS"), data.getStringExtra("TO_ADDRESS"), new BigDecimal("-" + data.getStringExtra("AMOUNT")).multiply(new BigDecimal("1000000000000000000")).toBigInteger());
                if (tabLayout != null)
                    tabLayout.getTabAt(2).select();
            }
        } else if (requestCode == rehanced.com.simplecoinwallet.activities.AppIntroActivity.REQUEST_CODE) {
            if (resultCode != RESULT_OK) {
                finish();
            } else {
                SharedPreferences.Editor editor = preferences.edit();
        ...
        } else if (requestCode == rehanced.com.simplecoinwallet.activities.SettingsActivity.REQUEST_CODE) {
    ...
    public void importPrivateKey(String privatekey){
        Intent genI = new Intent(this, rehanced.com.simplecoinwallet.activities.WalletGenActivity.class);
        genI.putExtra("PRIVATE_KEY", privatekey);
        startActivityForResult(genI, rehanced.com.simplecoinwallet.activities.WalletGenActivity.REQUEST_CODE);
    }
    ...
    private void selectItem(int position) {
        switch (position) {
            ...
            case 2: {
                Intent settings = new Intent(this, rehanced.com.simplecoinwallet.activities.SettingsActivity.class);
                startActivityForResult(settings, rehanced.com.simplecoinwallet.activities.SettingsActivity.REQUEST_CODE);
                break;
            }
            case 3: {
                new AlertDialog.Builder(this)
                        .setTitle("About FxNow")
                        .setMessage("FxNow is a blockchain.\n" +
                                    "It provides a coin.\n" +
                                    "Copyright@2017.")
            ...
            case 4: {
                ...
                i.setData(Uri.parse("https://www.fxnow.com/fxnow"));
                startActivity(i);
                break;
            }
            case 5: {
    ...
    public void broadCastDataSetChanged(){
        if(fragments != null && fragments[0] != null && fragments[1] != null) {
            ((FragmentWallets) fragments[0]).notifyDataSetChanged();
            ((FragmentTransactionsAll) fragments[1]).notifyDataSetChanged();
        }
    }
